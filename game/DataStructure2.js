DataStructure = function(xmax,ymax,zmax){
  ...
  this.unfacedStart;//unfacedStart.next
...
DataStructure.prototype = {
  ...
  setFromCubeArray: function(cubes){
    ...
    this.unfacedStart=initialpoint;
    for(var i=0;i<len;i++){
      cube=cubes[i];
      x=cube.x;y=cube.y;z=cube.z;
      pointer=this.point[x][y][z];
      pointer.type=cube.type;
      this.updateSums(x,y,z,1);
      //creates Linked List
      if(i==len-1){
        pointer.next=this.end;
        this.end.previous=pointer;
      }
      else{
        cubeNext=cubes[i+1];
        pointer.next=this.point[cubeNext.x][cubeNext.y][cubeNext.z];
  ...
  resetLogical: function(){
    this.faceCount=0;
    this.unfacedStart=this.start.next;
  ...
  updateOffsets: function(){
    var facesRemaining=this.faceCount,
    facechunk,start,index,count,
    facechunkNumber=0,
    offsets=new Array();
    while(facesRemaining>16384){
      start=facechunkNumber*98304;
      index=facechunkNumber*65536;
      count=16384*6;
      facechunkNumber++;
      offsets.push({
        start:start,
      ...
      facesRemaining-=16384;
    }
    start=facechunkNumber*98304;
    index=facechunkNumber*65536;
    count=facesRemaining*6;
    offsets.push({
      start:start,
      index:index,
      count:count
  ...
  addUnfacedCube: function(x,y,z,type){//add unfaced cubes to the end
    pointer=this.point[x][y][z];
    pointer.type=type;
    if(this.unfacedStart===this.end)
      this.unfacedStart=pointer;
    pointer.next=this.end;
    pointer.previous=this.end.previous;
    this.end.previous=pointer;
    pointer.previous.next=pointer;
    this.updateSums(x,y,z,1);
  },
  addCube: function(x,y,z,type){//add faced cubes to the beginning
    var pointer=this.point[x][y][z];
    pointer.type=type;
    this.updateSums(x,y,z,1);
    pointer.next=this.start.next;
    pointer.previous=this.start;
    this.start.next=pointer;
    pointer.next.previous=pointer;
    if(!pointer.ZM||!pointer.ZM.type)//either no point or point with no type
      this.addZMFace(pointer);
    else
      this.removeFace(pointer.ZM.ZPface);
    if(!pointer.ZP||!pointer.ZP.type)
      this.addZPFace(pointer);
    else
      this.removeFace(pointer.ZP.ZMface);
    if(!pointer.YM||!pointer.YM.type)
      this.addYMFace(pointer);
    else
      this.removeFace(pointer.YM.YPface);
    if(!pointer.YP||!pointer.YP.type)
      this.addYPFace(pointer);
    else
      this.removeFace(pointer.YP.YMface);
    if(!pointer.XM||!pointer.XM.type)
      this.addXMFace(pointer);
    else
      this.removeFace(pointer.XM.XPface);
    if(!pointer.XP||!pointer.XP.type)
      this.addXPFace(pointer);
    else
      this.removeFace(pointer.XP.XMface);
  },
  
  removeUnfacedCube: function(pointer){
    if(!pointer.type)return;
    if(this.unfacedStart===pointer)
      this.unfacedStart=pointer.next;
